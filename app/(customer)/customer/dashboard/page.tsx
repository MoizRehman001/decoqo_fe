...
import { DashboardStatsGrid } from '@/components/customer/DashboardStats';
import { RecentActivity } from '@/components/customer/RecentActivity';
import { useAuthStore } from '@/lib/stores/auth.store';
import { useProjects, useDashboardStats } from '@/lib/api/projects';
import type { Project } from '@/types/project.types';
import type { DashboardStats } from '@/types/project.types';

// ---------------------------------------------------------------------------
// Skeleton fixtures — mirror real layout for boneyard bone capture
// ---------------------------------------------------------------------------
  ...
  return (
    <div className="space-y-4 animate-pulse">
      {Array.from({ length: 3 }).map((_, i) => (
        <div key={i} className="ivory-card rounded-2xl p-5 flex items-start gap-3">
...
export default function CustomerDashboardPage() {
  ...
  const { data: projectsResult, isLoading: projectsLoading, isError: projectsError } = useProjects();
    ...
    <div className="space-y-8 animate-page-in">
      ...
      <section aria-labelledby="stats-heading">
        ...
        <Skeleton
          ...
          animate="shimmer"
          transition={300}
          fixture={<StatsSkeleton />}
        >
          {stats ? <DashboardStatsGrid stats={stats as DashboardStats} /> : null}
        </Skeleton>
      </section>

      {/* Main grid */}
      <div className="grid gap-6 lg:grid-cols-3">
        {/* Recent Projects — boneyard skeleton */}
        <section className="lg:col-span-2" aria-labelledby="recent-projects-heading">
          <div className="mb-4 flex items-center justify-between">
            ...
            <Link href="/customer/projects" className="text-sm text-accent underline underline-offset-2 hover:text-accent/80">
              View all
            </Link>
          </div>

          <Skeleton
            name="customer-dashboard-projects"
            loading={projectsLoading}
            animate="shimmer"
            transition={300}
            fixture={<ProjectsSkeleton />}
          >
            {projectsError ? (
              ...
              <div className="neu-card-3d rounded-2xl p-8 text-center">
                ...
                <h3 className="font-serif text-lg font-semibold text-foreground">Start your first project</h3>
      ...
      <section aria-labelledby="cta-heading">
        <div
          ...
          style={{ background: 'linear-gradient(135deg, hsl(0 0% 5%) 0%, hsl(0 0% 8%) 100%)' }}
        >
          <div
            className="pointer-events-none absolute inset-0 opacity-30"
            style={{ background: 'radial-gradient(ellipse 60% 80% at 80% 50%, hsl(45 65% 52% / 0.2), transparent)' }}
            aria-hidden="true"
          />
